...
public class RevisedHistoricalEvent extends HistoricalEvent {
	...
	*/
	...
	public RevisedHistoricalEvent() {
		this("",HistoricalEvent.DEFAULT_LOCATION, HistoricalEvent.DEFAULT_DESCRIPTION,HistoricalEvent.DEFAULT_EVENT_DAY,DEFAULT_REVISED_DESCRIPTION,DEFAULT_CITATION);
	}
	...
	*/
	...
	public RevisedHistoricalEvent(RevisedHistoricalEvent original) {
		if (original != null) {
			this.setAll(original.getImageName(), original.getLocation(), original.getDescription(), original.getEventDay(), original.revisedDescription, original.citation);
		} else {
	...
	public RevisedHistoricalEvent(String[] historicalData) {
	...
	*/
	...
	public boolean setAll(String[] values) {
		if (values.length >= 6) {
			setAll(values[0], values[1], values[2], values[3], values[4], values[5]);
			return true;
		}
		return false;
	}

	public boolean setAll(String imageName, String location, String description, String eventDay, String revisedDescription, String citation) {
		return setAll(imageName, location, description, new Date(eventDay), revisedDescription, citation);
	}
}